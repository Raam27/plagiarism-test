...
func doubleCalculatorWorker(queue chan request, maxThroughput int, maxObservedThroughtputC chan int) {
	...
	//kita akan pelajari bagian2tex di sesi berikutnya
	//secara sederhananya bagian2tex digunakan untuk memastikan hanya ada satu goroutine yang mengakses suatu bagian kode
	bagian2 := &sync.bagian2tex{}
	for req := range queue {
		//beginanswer
		semaphore <- 1
		//endanswer
